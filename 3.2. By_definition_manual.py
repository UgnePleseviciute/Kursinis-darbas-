...
if rank == 0:
    ...
    has_description = full_df['description_t'].fillna('').str.strip().astype(bool)
    with_desc = full_df[has_description].copy().sort_values('combined_score').head(355)
    without_desc = full_df[~has_description].copy().sort_values('combined_score').head(514)
    ...
    with_desc[cols_to_save].to_csv(os.path.join(COMBINED_OUTPUT_DIR, "lowest355_with_desc.csv"), index=False)
    without_desc[cols_to_save].to_csv(os.path.join(COMBINED_OUTPUT_DIR, "lowest514_without_desc.csv"), index=False)
